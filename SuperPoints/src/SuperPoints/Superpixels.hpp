...
//				+ opt.weight_world*d_world
		...
		inline S DistanceSpatial(const Point& x, const Point& y, const ParametersExt& opt) {
			S d_point;
			if(cUseSqrt) {
		...
		inline S DistanceSpatial(const Cluster& x, const Cluster& y, const ParametersExt& opt) {
			return DistanceSpatial(x.center, y.center, opt);
		}
...
//				const Danvil::Images::Image3fPtr& points,
		...
		void PlotEdges(const std::vector<int>& point_labels, const Danvil::Images::Image3ubPtr& img, unsigned int edge_w, unsigned char edge_r, unsigned char edge_g, unsigned char edge_b);
